...
public class PPAuto extends CommandBase {
    // PathPlannerTrajectory path = PathPlanner.loadPath("LCube-LCubePick-MCube-Balance", new PathConstraints(4, 3));
    ...
    public void initialize() {
        ...
        if(resetOdometry) {
            new Rotation2d();
            RobotContainer.driveTrain.resetOdometry(new Pose2d(initialPose.getTranslation(), Rotation2d.fromDegrees(RobotContainer.driveTrain.getHeading())));
        }
    }
    ...
    public void execute() {
        ...
        ChassisSpeeds targetSpeeds = controller.calculate(RobotContainer.driveTrain.getPose(), desiredState, new Rotation2d(desiredState.holonomicRotation.getRadians()));
        ...
        RobotContainer.driveTrain.drive(targetSpeeds.vxMetersPerSecond, targetSpeeds.vyMetersPerSecond, targetSpeeds.omegaRadiansPerSecond, true);
    }
