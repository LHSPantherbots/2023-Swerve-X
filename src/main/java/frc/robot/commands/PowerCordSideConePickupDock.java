...
public class PowerCordSideConePickupDock extends SequentialCommandGroup {
  public PowerCordSideConePickupDock(
    ...
    PathPlannerTrajectory path =
        PathPlanner.loadPath("PowerCordSideCubePickupDock", new PathConstraints(3, 2), false);
    HashMap<String, Command> eventMap = new HashMap<>();
    // eventMap.put("event1", new RunCommand(led::bluePulse, led));
    eventMap.put(
        "event1",
        new ConeIntakeGround(crossslide, intakepivot, elevator)
            .alongWith(new RunCommand(intake::intakeCone, intake).withTimeout(2.0))
            .andThen(
