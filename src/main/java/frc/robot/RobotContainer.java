...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    elevator.setDefaultCommand(
        // new RunCommand(() -> elevator.manualElevator(-operatorController.getLeftY()*.1),
        // elevator)
    ...
    crossSlide.setDefaultCommand(
        // new RunCommand(() -> crossSlide.manualCrossSlide(-operatorController.getLeftX()*.1),
        // crossSlide)
        new FunctionalCommand(
            // Reset controller on command start
    ...
    intakePivot.setDefaultCommand(
        // new RunCommand(() -> intakePivot.manualintakePivot(operatorController.getRightY()*.2),
        // intakePivot)
        new FunctionalCommand(
            // Reset controller on command start
  ...
  private void configureButtonBindings() {
    ...
    new POVButton(operatorController, GamePadButtons.Right)
        .onTrue(
            new ConditionalCommand(
                new ConeScoreMid_OLD(crossSlide, intakePivot, elevator), // runs if cone mode
                new CubeScoreMid_OLD(
                    crossSlide, intakePivot, elevator), // runs if cube mode (or cone mode false)
                () -> robotState.getConeMode()));
